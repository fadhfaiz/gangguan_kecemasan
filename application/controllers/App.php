		...
		switch($page) {
			...
			case 4:
				...
				foreach($get_hasil as $kunci => $dt) {
					...
					# TODO : SETUP KERNEL LAMA DAN BARU
					$kernel = "lama"; // setup kernel di sini
						...
						foreach($penyakit_ids as $pi) {
							...
							if(!in_array($pi, $inserted_kategori)) {
								...
								if($kernel == "lama") {
									...
									$new_kombinasi_mb = $last_kombinasi + $kombinasi * (1 - $last_kombinasi);
									$new_kombinasi_md = $last_kombinasi + $kombinasi * (1 - (1 - $last_kombinasi));
									$new_kombinasi = $new_kombinasi_mb - $new_kombinasi_md;
